...
The Apple Command key {commandkey} graphic can be alternatively be used instead of using the kbd:[Command] short cut graphic. +
To enable its use, it first must be declared as a variable/attribute in the page header metadata. 
...
The variable _commandkey_ can now be used in brackets as a html entity to display the Apple Command key.

_Example Asciidoc text for the Apple Command key_
----
kbd:[{commandkey}] - *Apple Command key!*
----
